	...
	if (this->extender.extend( vertex_nearest->state, state_sample,
		...
		if (check_extended_trajectory_for_collision( vertex_nearest->state,
			...
			if (parameters.get_phase() >= 1) {  // Check whether phase 1 should occur.
				...
				// Create a copy of the final state
				state_extended = new state_t(*(trajectory_parent->list_states.back()));

				/* Compute the set of all nodes that reside in a ball of
				   a certain radius centered at the extended state */
				this->distance_evaluator.find_near_vertices_r( state_extended,
															   radius,
				...
				for (typename list<void*>::iterator iter = list_vertices_in_ball.begin();
					...
					if (this->extender.extend (vertex_curr->state, state_extended,
						...
						if ((exact_connection == 1)
							&& (check_extended_trajectory_for_collision(vertex_curr->state,trajectory_curr) == 1)) {
