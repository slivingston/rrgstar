...
int System::getTrajectory (State& stateFromIn, State& stateToIn, list<double*>& trajectoryOut) {
  ...
  cout << "NumDim: " << numDimensions << endl;
  ...
  double *dists = new double[numDimensions];
  for (int i = 0; i < numDimensions; i++) 
    dists[i] = stateToIn.x[i] - stateFromIn.x[i];
  ...
  for (int i = 0; i < numSegments; i++) {
    ...
    double* stateArrCurr = new double[numDimensions];
    for (int i = 0; i < numDimensions; i++)
  ...
  double* stateArrTo = new double[numDimensions];
  for (int i = 0; i < numDimensions; i++) 
    stateArrTo[i] = stateToIn[i];
  trajectoryOut.push_front (stateArrTo);
